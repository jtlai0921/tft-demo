...
//--- 填色
extern const unsigned short img1[];

void tft_img(uint16_t color, uint16_t x, uint16_t y, uint16_t w, uint16_t h){
  int k;
	BlockWrite(x, x+w-1, y, y+h-1);
	for(int i=0; i<w; i++){
		for(int j=0; j<h; j++){
      k=j*w+i;
			WriteData(img1[k]>>8); WriteData(img1[k]);
		}
	}
}
//--- 印字
void tft_test(void){
	...
	tft_color(0x1111);
	//---
//	for(int i=1,j=0x1111;i<1000,j<0xF100;i++,j++)
//	{	
	tft_img(0x6666,10,10,48,48);
  while(1){}
tft_box(0x6666,10,10,50,50);
  tft_box(0x6666,10,10,50,50);
	 tft_box(0x6666,70,10,50,50);
	 tft_box(0x6666,10,70,50,50);
  tft_box(0x6666,70,70,50,50);
	}
//	//---

...
uint16_t g=0x1117 ;
void tft_line2(uint16_t y0, uint16_t y1, uint16_t x){	
	BlockWrite(x, x, y0, y1);
for (uint16_t i=1; i<(5); i++)

	{
		WriteData(g>>i); WriteData(g);
	}
}
//--- 填色
...
void tft_box1(uint8_t color, uint8_t x, uint8_t y, uint8_t w, uint8_t h){
	BlockWrite(x, x+w-1, y, y+h-1);
	for(int i=0; i<w; i++){
		for(int j=0; j<h; j++){
			WriteData(color>>1); WriteData(color);
		}
	}
}
//--- 印字
...
for(int i=0x1100,j=0; i<0xaa1f,j<200; j++,i++){
	tft_box(i, j, j,10, 10);delay_ms(10);
}
	tft_color(0x0000);
	
	BlockWrite(60, 65, 10, 25);
	//---
//	int c = 'F' - ' ';
...
//	}
	...
	tft_box(aaaa[i], 0, 0,aaaa[i], aaaa[i]);
	
	{
		for(int j=270; j<310; j++)
	{
	    tft_line(40, 0,j);	delay_ms(10);	
	}
	for(int i=40; i<60; i++)
	{
		for(int j=270; j<280; j++)
	...
	for(int i=160,j=310; i<170,j>290; j--,i++)
	{	
	    tft_line(i, 0,j);	delay_ms(10);
	}
	for(int i=200,j=310; i>180,j>270; j--,i--)
	{	
	    tft_line(i, 0,j);	delay_ms(10);
	}
	
	///////////////////////////////////////////////
	for(int j=170; j<210; j++)
	{
	    tft_line2(1, 0,j);	delay_ms(10);	
	}
	for(int i=1; i<20; i++)
	{for(int j=170; j<180; j++)
	    tft_line2(i, 0,j);		delay_ms(10);
	}
//	for(int i=1; i<20; i++)
//	{for(int j=285; j<295; j++)
//	    tft_line(i, 0,j);		delay_ms(10);
...
//	}
	...
	for(int j=170; j<210; j++)
	{
	    tft_line2(120, 0,j);		delay_ms(10);
	}
	for(int i=129,j=190; i<140,j<210; j++,i++)
	{	
	    tft_line2(i, 0,j);	delay_ms(10);
	}
	for(int i=129,j=190; i<140,j>170; j--,i++)
	{	
	    tft_line2(i, 0,j);	delay_ms(10);
	}
for(int i=160,j=210; i<170,j>190; j--,i++)
	{	
	    tft_line2(i, 0,j);	delay_ms(10);
	}
	for(int i=200,j=210; i>180,j>170; j--,i--)
	{	
	    tft_line2(i, 0,j);	delay_ms(10);	
	}

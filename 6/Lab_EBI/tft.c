...
extern const unsigned short img3[];
void tft_img3(uint16_t color, uint16_t x, uint16_t y, uint16_t w, uint16_t h){
  int k;
	BlockWrite(x, x+w-1, y, y+h-1);
	for(int i=0; i<w; i++){
		for(int j=0; j<h; j++){
      k=i*h+j;
			WriteData(img3[k]>>8); WriteData(img3[k]);
		}
	}
}
extern const unsigned short img4[];
void tft_img4(uint16_t color, uint16_t x, uint16_t y, uint16_t w, uint16_t h){
  int k;
	BlockWrite(x, x+w-1, y, y+h-1);
	for(int i=0; i<w; i++){
		for(int j=0; j<h; j++){
      k=i*h+j;
			WriteData(img4[k]>>8); WriteData(img4[k]);
		}
	}
}
//--- 印字
...
void tft_test(void){
  ...
  tft_img2(0x6666,0,100,150,150);
    tft_img3(0x6666,160,80,156,159);
    tft_img4(0x6666,0,0,100,59);
 // while(10){}
 for(int i=0,j=0;i<50,j<30;i++,j++)   
{tft_box(0xffff, 110, 30, i, j);}
    py+=pd;
    y+=d;
    if(py>100){pd=-10;}
    if(py<1){pd=10;}
